...
\begin{center}
    \includegraphics[width=11cm]{hog-diagram.png}
\end{center}

\emph{Note: There is a feature descriptor for each block. Since HOG outputs a histogram of size 9 per cell, you can append those together to get the feature descriptor for each block. As a result of this, if you have (z,z) cells per block, the feature descriptor for each block will be of size z*z*9. Also, the blocks are overlapping as displayed in the diagram.}

\emph{Note: You may find it useful to look at the thesis of Navneet Dalal (who is the inventor of HOG) for a deeper dive into this topic. \href{http://lear.inrialpes.fr/people/dalal/NavneetDalalThesis.pdf}{[Link to thesis]} (pages 39, 41 in section 4.3 specifically).}

\begin{enumerate}[(a)]
\item Given an image of dimmensions \colorbox{lightgray}{72x72}, calculate the number of cells, blocks, and feature vector size that will occur if performing hog with the following parameters.

Scenario 1: Given, pixels per cell = \colorbox{lightgray}{4x4}, cells per block = \colorbox{lightgray}{4x4} (\emph{like your implementation of SIFT!}). Calculate:
\\ 
Number of cells: 
\\
Number of blocks: 
...
Scenario 2: Given, pixels per cell = \colorbox{lightgray}{8x8}, cells per block = \colorbox{lightgray}{2x2}. Calculate:
\\ 
Number of cells:
\\
Number of blocks: 
